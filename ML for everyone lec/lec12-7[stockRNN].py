...
for i in range(iteration):
    cost_val, _, out= sess.run([cost, train, output], feed_dict={X: x_train, y: y_train})
    if i % 300 == 0:
        print(cost_val)

# predict #
y_hat_train = sess.run(Y_pred, feed_dict={X: x_train})
y_hat = sess.run(Y_pred, feed_dict={X: x_test})
RMSE_train = sess.run(MSE, feed_dict={targets: y_train, predicts: y_hat_train})
RMSE = sess.run(MSE, feed_dict={targets: y_test, predicts: y_hat})
