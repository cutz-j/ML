...
def gradientDescent(X, W, y, myld, learning_rate=0.006, iteration=2000):
    m = len(X)
...
def normalize(X):
    ...
    stored_feature_means = stored_feature_means.reshape(1, len(stored_feature_means))
    Xnorm[:,1:] = Xnorm[:,1:] - stored_feature_means[:, 1:]
    stored_feature_stds = np.std(Xnorm, axis=0, ddof=1)
    stored_feature_stds = stored_feature_stds.reshape(1, len(stored_feature_stds))
...
new_w = gradientDescent(X_norm, init_w, y, myld=100)
plotFit(new_w, means, stds)
X_cv_norm, means_cv, stds_cv = normalize(poly(X_cv, 5))
poly_learning_curve(X_norm, X_cv_norm, y, y_cv, init_w, myld=0)
...
new_w = gradientDescent(X_norm, init_w, y, myld=2)
x_test_norm, means, stds = normalize(poly(X_test, 5))
print(cost(x_test_norm, new_w, y_test, myld=0))